...
const actionStyle: Record<string, string> = {
  ...
  INIT: "text-zinc-400 bg-zinc-700/30 border-zinc-600/30",
};
          ...
          {rows.map((row) => {
            ...
            return (
              <tr key={row.id} className="group hover:bg-zinc-800/30 transition-colors">
                <td className="py-3 pr-4 text-zinc-400 whitespace-nowrap">{row.tradedAt}</td>
                <td className="py-3 pr-4">
                  <div className="flex flex-col">
                    <span className="text-white font-mono text-xs font-semibold">{row.ticker}</span>
                    <span className="text-zinc-600 text-xs">{row.name}</span>
                  </div>
                </td>
                <td className="py-3 pr-4">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-bold border ${actionStyle[row.action] ?? "text-zinc-400"}`}>
                    {row.action}
                  </span>
                </td>
                ...
                <td className="py-3 pr-4">
                  {row.emotionTag && (
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs ${emotionStyle[row.emotionTag] ?? "text-zinc-400 bg-zinc-700/30"}`}>
                      {row.emotionTag}
                    </span>
                  )}
                </td>
        ...
        {rows.map((row) => {
          ...
          return (
            <Link key={row.id} href={`/journal/${row.id}`}>
              <div className="bg-zinc-800/40 border border-zinc-700/50 rounded-xl p-4 hover:border-zinc-600 transition-colors">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <span className="text-white font-mono font-semibold text-sm">{row.ticker}</span>
                    <span className="text-zinc-500 text-xs ml-2">{row.tradedAt}</span>
                  </div>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-bold border ${actionStyle[row.action] ?? "text-zinc-400"}`}>
                    {row.action}
                  </span>
